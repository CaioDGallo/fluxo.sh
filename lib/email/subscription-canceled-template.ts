...
export interface SubscriptionCanceledEmailData {
  planName: string;
  accessUntilDate: string; // formatted date
  appUrl: string;
  locale?: Locale;
}
...
export function generateSubscriptionCanceledHtml(
  ...
  const title = t('emails.subscriptionCanceled.title');
  const greeting = t('emails.subscriptionCanceled.greeting');
  const intro = t('emails.subscriptionCanceled.intro', {
    plan: data.planName,
    date: data.accessUntilDate,
  });
  const accessUntil = t('emails.subscriptionCanceled.accessUntil');
  const whatHappens = t('emails.subscriptionCanceled.whatHappens');
  const point1 = t('emails.subscriptionCanceled.point1');
  const point2 = t('emails.subscriptionCanceled.point2');
  const point3 = t('emails.subscriptionCanceled.point3');
  const reactivate = t('emails.subscriptionCanceled.reactivate');
  const footerText = t('emails.subscriptionCanceled.footer');
  ...
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: ${COLORS.card}; border: 2px solid ${COLORS.border}; padding: 20px; margin-bottom: 24px;">
      ...
      <div style="background: ${COLORS.bg}; border: 1px solid ${COLORS.border}; padding: 16px; margin-bottom: 16px;">
        <p style="margin: 0; color: ${COLORS.muted}; font-size: 12px;">${escapeHtml(accessUntil)}</p>
        <p style="margin: 8px 0 0 0; color: ${COLORS.fg}; font-weight: 700; font-size: 16px;">${escapeHtml(data.accessUntilDate)}</p>
      </div>
      ...
      <ul style="margin: 0; padding-left: 20px; color: ${COLORS.fg};">
        <li style="margin: 4px 0;">${escapeHtml(point1)}</li>
        <li style="margin: 4px 0;">${escapeHtml(point2)}</li>
        <li style="margin: 4px 0;">${escapeHtml(point3)}</li>
      </ul>
    </div>
...
export function generateSubscriptionCanceledText(
  data: SubscriptionCanceledEmailData
): string {
  const locale = data.locale ?? defaultLocale;
  const t = (key: string, params?: Record<string, string | number>) =>
...
${accessUntil}: ${data.accessUntilDate}
...
${reactivate}: ${data.appUrl}/settings/billing
