...
function getAccountMenuButton(page: Page, name: string) {
  return page
...
test('edit account name', async ({ page }) => {
  ...
  const accountMenuButton = getAccountMenuButton(page, ORIGINAL_NAME);
  await accountMenuButton.click(); // Three dots menu
...
test('delete account', async ({ page }) => {
  ...
  // Find the account card and open dropdown
  const accountMenuButton = getAccountMenuButton(page, ACCOUNT_NAME);
...
test('edit category name', async ({ page }) => {
  ...
  // Edit dialog should open
  const editDialog = page.getByRole('dialog', { name: 'Editar Categorias' });
  await expect(editDialog).toBeVisible();
  await expect(editDialog.getByLabel('Nome')).toHaveValue(ORIGINAL_NAME);
...
test('edit income', async ({ page }) => {
  ...
  // Edit dialog should open
  const editDialog = page.getByRole('dialog', { name: 'Editar Receita' });
  await expect(editDialog).toBeVisible();
  await expect(editDialog.getByLabel('Descrição')).toHaveValue(ORIGINAL_DESC);
